...
-- SELECT candidate.ID, candidate.first_name, candidate.last_name, count(ID) AS '# of Votes' FROM candidate INNER JOIN votes_in ON candidate.ID = votes_in.candidate_ID where votes_in.election_ID = 1 GROUP BY ID;
